...
from ..helpers.utils import _himitools, _himiutils, _format, install_pip, reply_id
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        mod.Config = Config
        mod._format = _format
        mod.tgbot = himiub.tgbot
        mod.sudo_cmd = sudo_cmd
        mod.CMD_HELP = CMD_HELP
        mod.reply_id = reply_id
        mod.admin_cmd = admin_cmd
        mod._himiutils = _himiutils
        mod._himitools = _himitools
...
def remove_plugin(shortname):
    ...
    try:
        name = f"userbot.plugins.{shortname}"
        for i in reversed(range(len(himiub._event_builders))):
            ev, cb = himiub._event_builders[i]
            if cb.__module__ == name:
                del himiub._event_builders[i]
    except BaseException:
        raise ValueError
